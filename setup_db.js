...
// ─── Step 4: Insert Verified Data from JSON ──────────────────────────────────
...
if (fs.existsSync(TNT_JSON_PATH)) {
  ...
  tntTx(tntData);
  console.log(`  Inserted ${tntData.length} verified TNT entries.`);
}
...
if (fs.existsSync(MARKET_JSON_PATH)) {
  ...
  marketTx(marketData);
  console.log(`  Inserted ${marketData.length} verified market rate overrides.`);
}
